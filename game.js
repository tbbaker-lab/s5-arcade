(() => {
  ...
  if(!location.search && location.hash.includes('access_token=')){
    const q=localStorage.getItem('s5_pending_query_v8');
  ...
  else if(!guildId){failAuth('The Discord server ID is missing. Create a fresh /game panel using the V8 bot.');}
  else if(!clientId){failAuth('The Discord application ID is missing. Create a fresh /game panel using the V8 bot.');}
  ...
  discordLoginBtn.onclick=()=>{
    ...
    localStorage.setItem('s5_oauth_state_v8',state);
    localStorage.setItem('s5_oauth_state_time_v8',String(Date.now()));
    localStorage.setItem('s5_pending_query_v8',location.search);
  ...
  async function finishDiscordAuth(){
    ...
    const expected=localStorage.getItem('s5_oauth_state_v8'),got=h.get('state');
    const stateTime=Number(localStorage.getItem('s5_oauth_state_time_v8')||0);
    ...
    try{
      ...
      localStorage.removeItem('s5_oauth_state_v8');localStorage.removeItem('s5_oauth_state_time_v8');localStorage.removeItem('s5_pending_query_v8');return true;
  ...
  function drawBird(t,dt){
    ...
    trail.push({x:x-18,y:y+rand(-3,3),life:.45,size:rand(5,14)});
    for(const p of trail)p.life-=dt;
    trail=trail.filter(p=>p.life>0).slice(-70);
    for(const p of trail){
      const lifeRatio=clamp(p.life/.45,0,1);
      const radius=Math.max(.15,p.size*lifeRatio);
      ctx.globalAlpha=lifeRatio*.22;
      ctx.fillStyle='#61ff20';
      ctx.beginPath();
      ctx.arc(p.x-(.45-p.life)*100,p.y,radius,0,Math.PI*2);
      ctx.fill();
    }
    ctx.globalAlpha=1;
