(() => {
  ...
  // V7: read the application ID from either the URL or the bot-signed challenge.
  // The last fallback is this S5 application's current ID so existing V6 panels also keep working.
  ...
  if(!ch){failAuth('The challenge data could not be read. Create a fresh /game panel and use its PLAY THE GAME button.');}
  else if(!challenge.includes('.')){failAuth('This is not a signed S5 challenge link. Use PLAY THE GAME in Discord.');}
  else if(!guildId){failAuth('The Discord server ID is missing from this challenge. Create a fresh /game panel.');}
  else if(!clientId){failAuth('The Discord application ID is missing. Update to the V7 website/bot files.');}
